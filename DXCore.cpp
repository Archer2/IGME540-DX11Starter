...
HRESULT DXCore::InitDirect3D()
{
	...
	// Determine if tearing ("vsync off") is available.
	// Necessary for variable refresh rate displays
	Microsoft::WRL::ComPtr<IDXGIFactory5> factory;
	if (SUCCEEDED(CreateDXGIFactory1(IID_PPV_ARGS(&factory)))) {
		// Check for specific feature
		BOOL tearingSupported = false;
		HRESULT featureCheck = factory->CheckFeatureSupport(DXGI_FEATURE_PRESENT_ALLOW_TEARING, &tearingSupported, sizeof(tearingSupported));
